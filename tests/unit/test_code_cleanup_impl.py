...
from stream.spark.main_job import run as spark_run
from common.connectors import AWSIAMConnector, OktaConnector, get_connector


def test_spark_run_references_kinesis_source() -> None:
    source = inspect.getsource(spark_run)
    assert ".format(\"kinesis\")" in source
    assert "cfg.kinesis.stream_name" in source
