...
def build_sql(*, start_date: date, end_date: date) -> tuple[str, str]:
    """Returns (delete_sql, insert_sql) for idempotent refresh."""
    ...
    delete_sql = "\n".join(
        [
            "DELETE FROM lakehouse.user_behavior_hourly",
            "WHERE window_size = '1h'",
            f"  AND window_start >= {start_ts}",
            f"  AND window_start < {end_ts};",
        ]
    )

    insert_sql = "\n".join(
        [
            "INSERT INTO lakehouse.user_behavior_hourly",
            "SELECT",
            ...
            "  (max(CASE WHEN has_new_device THEN 1 ELSE 0 END) = 1) AS has_new_device",
...
def run(*, config_path: str, start_date: date, end_date: date) -> None:
    ...
    print(
        f"[user_behavior_hourly_job] Refreshing {start_date.isoformat()} → {end_date.isoformat()} "
